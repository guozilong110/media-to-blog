...
**Active skin: transcripts blog.** Tokens below are matched to the `guozilong110/transcripts` GitHub Pages site so diagrams sit on the page without a visual seam. Source of truth for the site is `.agents/skills/media-to-blog/templates/post.html`; change both together.

The shipped default was a cool editorial palette (white-smoke paper, jet-black ink, atomic-tangerine accent, blue-slate muted). Swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#fafafa` | `#16201e` |
| `paper-2` | Diagram container bg, secondary fill | `#f2f4f4` | `#1e2b28` |
| `ink` | Primary text, primary stroke | `#1d1d1f` | `#f2f4f3` |
| `muted` | Secondary text, default arrow stroke | `#5b6b68` | `#a8b5b2` |
| `soft` | Sublabels, boundary labels | `#7e8b89` | `#8b9895` |
| `rule` | Hairline borders | `rgba(29,29,31,0.12)` | `rgba(242,244,243,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#e3e3e6` | `rgba(242,244,243,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#0b6b5f` (deep teal) | `#4fbfa8` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(11,107,95,0.08)` | `rgba(79,191,168,0.12)` |
| `link` | HTTP/API calls, external arrows | `#1f5673` | `#6fa8c9` |

> **Brand palette source:** taken from the transcripts site CSS variables — `--ink #1d1d1f`, `--line #e3e3e6`, `--accent #0b6b5f`, page background `#fafafa`, table header `#f2f4f4`. `muted`, `soft`, and `link` are derived to cover roles the site does not name. Contrast on `#fafafa`: ink 16.1:1, muted 5.4:1, accent 6.1:1, link 7.6:1. On dark `#16201e`: ink 15.1:1, muted 7.9:1, accent 7.4:1. All clear WCAG AA.
>
> **Deliberate deviation:** the "paper is warm-neutral" constraint below is waived. The site uses neutral `#fafafa`, and matching it matters more than warmth.
...
Any `rgba(29,29,31, X)` in light becomes `rgba(242,244,243, X)` in dark. Same opacities, RGB flipped. The accent gets a slight hue-shift brighter to read on dark paper.
...
> **Simplified Chinese is the default label language for this skin.** Diagrams embed in Chinese-language blog posts, so most labels are Han. Apply the three Han rules in `### Traditional Chinese labels` below, with the Simplified stack. Concretely: `<text font-family="'Geist', 'Noto Sans SC', 'PingFang SC', 'Microsoft YaHei', sans-serif">` for labels, `'Instrument Serif', 'Noto Serif SC', serif` for titles, 12px floor, and eyebrow/arrow-label/legend slots switch to 12px sans weight 500 with no tracking and no uppercase. Keep Latin sublabels (ports, units, numbers) in Geist Mono.
...
```html
<link href="https://fonts.googleapis.com/css2?family=Instrument+Serif:ital@0;1&family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500;600&family=Noto+Sans+SC:wght@400;500;600&family=Noto+Serif+SC:wght@400&family=Noto+Sans+KR:wght@400;500;600&family=Noto+Serif+KR:wght@400&family=Noto+Sans+TC:wght@400;500;600&family=Noto+Serif+TC:wght@400&display=swap" rel="stylesheet">
```

Noto Sans SC and Noto Serif SC are added to the link for this skin, so Simplified labels resolve to the web font instead of the viewer's local face. `css2` slices Han by unicode-range, so a diagram with a dozen Chinese labels downloads only the slices it uses.

**Blog embedding:** posts are self-contained and load no external resources. A diagram embedded into a post therefore drops this `<link>` and relies on the local stack (`'PingFang SC', 'Microsoft YaHei'` for Han, `-apple-system, BlinkMacSystemFont` for Latin, `ui-monospace, SFMono-Regular, Menlo` for mono). Standalone diagram files keep the link.
...
Simplified Chinese takes the same three rules with the Simplified stack (`'Noto Sans SC'`, `'PingFang SC'`, `'Microsoft YaHei'`). This skin adds both Noto SC faces to the font link above, so standalone Simplified diagrams resolve the web font; posts that strip the link fall back to the local stack.
...
- **Paper is warm-neutral, not pure white**: pure white turns the design sterile. Pick a cream, bone, or light grey with a hint of warmth. *Waived in the active skin* — it matches the site's neutral `#fafafa` instead.
